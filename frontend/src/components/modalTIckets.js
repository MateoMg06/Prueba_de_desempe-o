...
export async function initModalTicket(refreshTickets) {
  ...
  btnNew.addEventListener('click', () => {
    ...
    btnSubmit.textContent = "Create Reservation";
  ...
  form.onsubmit = async (e) => {
    ...
    const ticket = {
      workspace: document.getElementById("ticket-title").value,
      reason: document.getElementById("ticket-desc").value,
...
function canEditTicket(user, ticket) {
  ...
  const isClosed = ticket.status == "Denied" || ticket.status == "Accepted";
